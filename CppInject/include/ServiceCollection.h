...
namespace CppInject
{
  ...
  public:
    ...
    TService& getRequiredService()
    {
      ...
      if (res != nullptr)
        return *res;
      throw std::logic_error(typeid(TService).name() + std::string{ " has not been registered as a singleton or scoped service." });
    }
    ...
    std::shared_ptr<TService> getRequiredTransientService()
    {
      ...
      if (res != nullptr)
        return res;
      throw std::logic_error(typeid(TService).name() + std::string{ " has not been registered as a transient service." });
    }
    ...
    ServiceVector<TService> getServices()
    {
      ...
      for (auto& service : servicesAny)
        res.emplace_back(std::any_cast<std::reference_wrapper<TService>>(service));
      return res;
    }
    ...
    std::vector<std::shared_ptr<TService>> getTransientServices()
    {
      ...
      auto servicesAny = getServices(std::type_index(typeid(std::shared_ptr<TService>)));
      for (auto&& service : std::move(servicesAny))
        res.emplace_back(std::any_cast<std::shared_ptr<TService>&&>(std::move(service)));
      return res;
    }
  };
  ...
  template<typename T>
  struct IsVector <T, typename std::enable_if_t<std::is_same<typename std::decay_t<T>, std::vector<typename T::value_type, typename T::allocator_type>>::value>>
    : std::true_type { };
  ...
  class ServiceFactory
  {
    ...
    {
      T services{};
      auto servicesAny = serviceProvider.getServices(std::type_index(typeid(std::decay_t<typename T::value_type::type>)));
      for (auto& service : servicesAny)
    ...
    template<class T>
    inline std::enable_if_t<IsSharedPointer<T>::value, T> getService(IServiceProvider& serviceProvider)
    {
      return std::any_cast<T>(serviceProvider.getService(std::type_index(typeid(T))));
    }

    template<class T>
    inline std::enable_if_t<!IsSharedPointer<T>::value && !IsVector<T>::value, T&> getService(IServiceProvider& serviceProvider)
    {
      return std::any_cast<std::reference_wrapper<T>>(serviceProvider.getService(std::type_index(typeid(T))));
    }
  ...
  template<class TService>
  std::shared_ptr<TService> ServiceFactory<TService>::create(IServiceProvider& serviceProvider)
  {
  ...
  struct ServiceDescription
  {
    ...
    ServiceDescription(FactoryFunction&& createFunc, ConversionFunction&& conversionFunc, ServiceType type)
  ...
  public:
    ...
    template<class TService, class TImplementation = TService, typename = typename std::enable_if_t<std::is_base_of_v<TService, TImplementation>>>
    void addSingleton();
  ...
  {
    const std::unordered_map<std::type_index, FactoryFunctionCollection> _factories;
    ...
    static std::any getService(
      ...
      std::unordered_map<std::type_index, std::vector<std::any>>& providerInstances,
      std::vector<std::any>& initializationOrder,
      IServiceProvider& serviceProvider,
      size_t index);

  public:
    ServiceProvider(std::unordered_map<std::type_index, FactoryFunctionCollection> factories)
  ...
  template<class TService, class TImplementation,
    ServiceType serviceType, typename = typename std::enable_if_t<serviceType != ServiceType::Transient>>
    void addService(std::unordered_map<std::type_index, FactoryFunctionCollection>& factories)
  {
    const auto typeIndex = std::type_index(typeid(TService));
    auto& serviceFactories = factories.emplace(typeIndex, FactoryFunctionCollection{}).first->second;
    serviceFactories.emplace_back(
      ...
      [](std::any managedData) -> std::any {
        return std::ref(static_cast<TService&>(*std::any_cast<std::shared_ptr<TImplementation>&>(managedData)));
      },
        serviceType);
  }
  ...
  void ServiceCollection::addTransient()
  {
    ...
    auto& factories = _factories.emplace(typeIndex, FactoryFunctionCollection{}).first->second;
    factories.emplace_back(
      ...
      [](std::any managedData) -> std::any {
        return std::static_pointer_cast<TService>(std::any_cast<std::shared_ptr<TImplementation>>(std::move(managedData)));
      },
    ...
    size_t index)
  {
    ...
    {
      auto instances = providerInstances.emplace(factories.first, std::vector<std::any>{});
      if (instances.second)
        instances.first->second.resize(factories.second.size());
      if (!instances.first->second.at(index).has_value())
      {
        auto it = instances.first->second.emplace(std::next(instances.first->second.begin(), index), desc.create(serviceProvider));
  ...
  std::vector<std::any> ServiceProvider::getServices(std::type_index type)
  {
    ...
    for (size_t i = 0; i < factoryIt->second.size(); ++i)
      res.emplace_back(getService(*factoryIt, _instances, _initializationOrder, *this, i));
    return res;
  }
  ...
  {
    _instances.clear();
    while (!_initializationOrder.empty())
      _initializationOrder.pop_back();
  }

  ServiceProvider::ScopedServiceProvider::ScopedServiceProvider(ServiceProvider& parent)
    : _parent(parent)
  { }
  ...
  std::any ServiceProvider::ScopedServiceProvider::getService(std::type_index type)
  {
    auto factoryIt = _parent._factories.find(type);
    if (factoryIt == _parent._factories.end())
      return std::any();
    const auto& desc = factoryIt->second.front();
    return ServiceProvider::getService(
      *factoryIt,
      desc.type == ServiceType::Scoped ? _instances : _parent._instances,
      desc.type == ServiceType::Scoped ? _initializationOrder : _parent._initializationOrder,
      *this,
      0);
  }

  std::vector<std::any> ServiceProvider::ScopedServiceProvider::getServices(std::type_index type)
  {
    std::vector<std::any> res;
    auto factoryIt = _parent._factories.find(type);
    if (factoryIt == _parent._factories.end())
